$(function(){
    ...
    // pager li의 클릭기능을 켬. click_slide함수 실행
    function click_slide(){
        ...
        if(i>k){
            $('.imgs li').eq(k).css('left','0').animate({'left':'-100%'},600); // 현재슬라이드
    ...
    function timer(){
        repeat = setInterval(function(){
            ...
            $('.imgs li').eq(i-1).css('left','0').animate({'left':'-100%'},600); // 현재슬라이드
    ...
    $('#img_slide').mouseover(function(){
        ...
        // #img_slide에 마우스를 올렸을때 setInterval기능을 끔
    });
    $('#img_slide').mouseleave(function(){
        timer();
        // #img_slide에 마우스를 뗐을때 setInterval이 지정되어있는 timer실행함수를 다시 실행시킴
    });
    ...
    function pressKey(e){
        if(e.keyCode == 65 || e.keyCode == 37){
            // pressKey 함수 내용(e라는 값에 키보드의 정보를 전부 저장함)
            // A || 왼쪽화살표 키를 눌렀을 때
            prev_slide();            
