...
function ProductsUserList(){
    ...
    return(
        <div>
            <h1>사용자 목록</h1>
            {dummyProducts.map((user,index)=>(
                <ProductsCard key={index} productName={user.productName} price={user.price} description={user.description} inStock={user.inStock}/>
            ))};
        </div>
    );
}
