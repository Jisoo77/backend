...
public class CoffeeService {
    ...
    // menu - 위의 두가지가 저장되어있는 맵 menu(coffeeList, coffeePrice);
    // orderList(커피이름, 수량)
    ...
    private void totalOrder(Customer customer2) {
        ...
        for(Map.Entry<String, Integer> order : customer.getCoffeeOrder().entrySet()){
            ...
            // coffee.menu.get(order.getKey()) 커피의 이름을 키로 가격을 값으로 갖는 map 객체 order.getKey() 해당커피의 가격
    ...
    private void payment(int totalMoney) {
        ...
        if(payResult < 0){
            System.out.println("잔액이 부족합니다. 확인후 다시 주문해주세요.");
            System.exit(0);
        }else{
