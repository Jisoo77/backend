...
// 회원 (Member) 객체에 '남자' 회원의 평균나이를
// 외부 반복자를 이용하는 방법과, 내부 반복자 람다식 스트림을 적용하는 방법으로 코딩하시오
public class Stream9 {
    public static void main(String[] args) {
        ...
        // 1. 외부반복자
        // index값을 사용 for문과 if문으로 남성/여성 구분
        // 남성 나이만 sum에 누적
        // count로 나눠서 남성 평균나이 출력
        int count = 0;
        double sum = 0;
        ...
        double ageAvg = list.stream()
            ...
            .average()
            .getAsDouble();
        System.out.println("남자 회원 평균 나이 : " + ageAvg);
    }
}
// 1. list.stream() : 리스트를 스트림변환
// 2. 성별이 남성인 회원만 필터링 -> 중간연산
// 3. 회원객체를 나이로 매칭 - mapToInt 메서드 객체를 int 값으로 변환
// -IntStream을 생성
// 4. average() -> 스트림 요소들의 평균을 계산 -> 최종처리 집계 처리
