...
public class Passenger { // 예매정보를 저장 - 나이검증 여기서
    private String name;
    private int birthDate; // 생년월일 yymmdd
    ...
    public boolean man15(Passenger p){ // 831016
        ...
        int m = (p.birthDate % 10000) / 100; // 831016/10000 나머지 1016/100 ->몫은 10만 남음
        ...
        LocalDate currenDate = LocalDate.now(); // 오늘 년월일
        int age = Period.between(birthDate2,currenDate).getYears();
        // Period.between(birthDate2,currenDate).getDays() -> 두 날짜간의 차이
        return age > 15;
    }
